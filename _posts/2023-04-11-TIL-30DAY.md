...
&emsp;CSS는 부모를 탐색할 수 없지만 JavaScript에서는 이벤트 흐름으로 부모요소를 탐색할 수 있어서 유연한 웹 페이지를 구현 가능하다
{:.notice--primary}
...
⚠️ 프로젝트가 보안 점검을 거치는 경우 innerHTML 를 사용하면 잠재적인 보안 위험성때문에 코드가 거부될 가능성이 높다고 한다
{:.notice--warning}
